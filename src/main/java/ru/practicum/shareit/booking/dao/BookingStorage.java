...
public interface BookingStorage extends JpaRepository<Booking, Long> {
    ...
    List<Booking> getCurrentBookingsByBooker(Long bookerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.booker.id = ?1 and b.end < ?2 " +
            "order by b.start desc ")
    List<Booking> getPastBookingsByBooker(Long bookerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.booker.id = ?1 and b.start > ?2 " +
            "order by b.start desc ")
    List<Booking> getFutureBookingsByBooker(Long bookerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.booker.id = ?1 and b.status = 'WAITING' and b.start > ?2 " +
            "order by b.start desc ")
    List<Booking> getWaitingBookingsByBooker(Long bookerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.booker.id = ?1 and b.status = 'REJECTED' " +
            "order by b.start desc ")
    List<Booking> getRejectedBookingsByBooker(Long bookerId);
    ...
    @Query(" select b from Booking b " +
            ...
            "order by b.start desc ")
    List<Booking> getCurrentBookingsByOwner(Long ownerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.item.owner.id = ?1 and b.end < ?2 " +
            "order by b.start desc ")
    List<Booking> getPastBookingsByOwner(Long ownerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.item.owner.id = ?1 and b.start > ?2 " +
            "order by b.start desc ")
    List<Booking> getFutureBookingsByOwner(Long ownerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.item.owner.id = ?1 and b.status = 'WAITING' and b.start > ?2 " +
            "order by b.start desc ")
    List<Booking> getWaitingBookingsByOwner(Long ownerId, LocalDateTime now);

    @Query(" select b from Booking b " +
            "where b.item.owner.id = ?1 and b.status = 'REJECTED' " +
            "order by b.start desc ")
    List<Booking> getRejectedBookingsByOwner(Long ownerId);
